...
Then we can decode those index into words as following:
```py
decoded_text = tokenizer.decode(out.squeeze(0).tolist())
print(decoded_text)
```
The output of above code would like following:
```py
I like to backbonePersonallyazes grit transmitiddled
```
The predicted words make no sense, that's because we haven't train the model, that would be the topic of next course
